...
function ViewRoleAndPermissionBody() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (response.data?.roles?.length > 0) {
        const formattedData = response.data.roles.map(role => ({
          id: role.id,
          roleName: role.roleName,
          permissions: role.permissions,
        }));
          ...
          {permissionDatas.create_role && (
            <div>
              <Link
                ...
                className="submit flex-none rounded-md px-3.5 py-2.5 text-sm font-semibold text-white shadow-sm focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 text-center whitespace-nowrap"
              >
                Create Permissions
              </Link>
            </div>
  ...
  <div className="overflow-x-auto">
    <table className="min-w-full bg-white border border-gray-200 text-xs">
      ...
      <tbody className="bg-white">
        {searchedRole.permissions && (() => {
          const isKitchen = searchedRole.permissions.isKitchenRole === true;
          const permissionEntries = Object.entries(searchedRole.permissions).filter(([key]) => key !== 'isKitchenRole');
          const totalRows = isKitchen ? 1 : permissionEntries.length;
          
          if (isKitchen) {
            return (
              <tr className='border-b-4 border-gray-300'>
                <td className="px-4 py-2 text-left text-gray-900 align-middle">
                  <div className="flex items-center gap-2">
                    {searchedRole.roleName}
                    <span className="inline-flex items-center gap-1 bg-[#E8F5F1] text-[#1A5B63] text-[10px] font-semibold px-2 py-0.5 rounded-full">
                      🍳 Kitchen Display
                    </span>
                  </div>
                </td>
                <td className="px-4 py-2 text-left text-gray-500 italic" colSpan="2">
                  Kitchen Display Login Only
                </td>
          ...
          return (
            <>
              {permissionEntries.map(([permissionKey, subPermissions], index) => {
                const isLastRow = index === totalRows - 1;
                return (
                  <tr key={`${searchedRole._id}-${permissionKey}`} className={isLastRow ? 'border-b-4 border-gray-300' : 'border-b border-gray-200'}>
                    {index === 0 && (
                      <td className="px-4 py-2 text-left text-gray-900 align-middle" rowSpan={totalRows}>
                        {searchedRole.roleName}
                      </td>
                    )}
                    <td className="px-4 py-2 text-left text-gray-900 align-top">
                      {permissionKey.replace(/([A-Z])/g, ' $1')}
                    </td>
                    <td className="px-4 py-2 text-left text-gray-900 align-top">
                      <div className="space-x-2">
                        {Object.entries(subPermissions).map(([subPermissionKey, isAllowed], idx, array) => (
                          <span 
                          ...
                          >
                            {subPermissionKey.replace(/([A-Z])/g, ' $1')}{idx < array.length - 1 ? ', ' : ''}
                          </span>
                        ))}
                      </div>
                    </td>
                    {index === 0 && (
                      <td className="px-4 py-2 text-gray-900 align-middle" rowSpan={totalRows}>
                        <div className='flex items-center'>
                          {permissionDatas.edit_role && (
                          <Link to={`/editPermissions/${searchedRole._id}`} className="text-blue-500 hover:text-blue-700 font-bold px-2 mr-2">
                            <i className="fas fa-edit"></i>
  ...
  <div className="overflow-x-auto">
    <table className="min-w-full bg-white border border-gray-200 text-xs">
      ...
      <tbody className="bg-white">
        {permissionData.map((Permission, roleIndex) => {
          const isKitchen = Permission.permissions?.isKitchenRole === true;
          const permissionEntries = Permission.permissions ? Object.entries(Permission.permissions).filter(([key]) => key !== 'isKitchenRole') : [];
          const totalRows = isKitchen ? 1 : permissionEntries.length;
          ...
          return (
            <>
              {permissionEntries.map(([permissionKey, subPermissions], index) => {
                const isLastRow = index === totalRows - 1;
                return (
                  <tr key={`${Permission.id}-${permissionKey}`} className={isLastRow ? 'border-b-4 border-gray-300' : 'border-b border-gray-200'}>
                    {index === 0 && (
                      <td className="px-4 py-2 text-left text-gray-900 align-middle" rowSpan={totalRows}>
                        {Permission.roleName}
                      </td>
              ...
              {/* Empty spacer row between roles */}
              <tr className="h-4 bg-gray-50">
                <td colSpan="4"></td>
              </tr>
            </>
