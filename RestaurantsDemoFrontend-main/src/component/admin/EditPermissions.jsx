...
const EditPermissionsBody = () => {
    ...
    useEffect(() => {
        ...
        const fetchPermissionData = async () => {
            try {
                ...
                setWarehouseData(currentWarehouses);

                let formattedPermissions = roleData.permissions || {};

                // Initialize managePOS if not exists
                ...
                // Check if this is a kitchen display role
                if (formattedPermissions.isKitchenRole === true) {
                    setIsKitchenRole(true);
                }
    ...
    // Handle warehouse permission change
    ...
    const isAllPermissionsSelected = () => {
        return categorySections.every(section => isSectionAllSelected(section.id));
    };
    ...
    const handleSubmit = (event) => {
        ...
        for (const [mainPermission, subPermissions] of Object.entries(permissions)) {
            ...
            for (const [subPermission, value] of Object.entries(subPermissions)) {
                updatedPermissions[mainPermission][subPermission] = value;
            }
        }

        // Process managePOS permissions
        ...
        if (isKitchenRole) {
            updatedPermissions.isKitchenRole = true;
        } else {
            delete updatedPermissions.isKitchenRole;
        }
