...
function BotSettingsBody() {
    ...
    useEffect(() => {
        if (settings?.logoUrl) {
            ...
            setLogoPreview(`${baseUrl}/api/bot-logo/${settings.logoPath.split('/').pop()}`);
        } else {
            setLogoPreview(null);
        }
    }, [settings]);
    ...
    const fetchSettings = async () => {
        try {
            const { data } = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/getBOTSettings`);
    ...
    // Tab configuration
    const tabs = [
        { id: 'general', label: 'General', icon: '⚙️', description: 'Basic BOT settings' },
    ...
    const handleLogoUpload = async (event) => {
        ...
        try {
            const response = await axios.post(
                `${process.env.REACT_APP_BASE_URL}/api/uploadBOTLogo`,
                formData,
            ...
            setSettings(prev => ({
                ...
                logoUrl: response.data.logoUrl || `${process.env.REACT_APP_BASE_URL}/api/bot-logo/${response.data.logoPath.split('/').pop()}`
            }));
    ...
    const handleRemoveLogo = async () => {
        ...
        try {
            await axios.post(`${process.env.REACT_APP_BASE_URL}/api/removeBOTLogo`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.post(`${process.env.REACT_APP_BASE_URL}/api/createOrUpdateBOTSettings`, settings);
                ...
                <div className="px-8 py-6">
                    <div className="flex justify-between items-center">
                        <div>
                            ...
                            <p className="text-gray-600 mt-1">Customize your 80mm thermal BOT layout and content</p>
                        </div>
                        <Link
                            to="/dashboard"
                ...
                <form onSubmit={handleSubmit}>
                            ...
                            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                                <div className="flex items-center justify-between mb-6">
                                    <div>
                                        ...
                                        <p className="text-gray-600">Configure basic BOT appearance and behavior</p>
                                    </div>
                                </div>
                    ...
                    {activeTab === 'header' && (
                        <SectionCard
                            ...
                            description="Configure what information appears in the BOT header"
                                            ...
                                            <div className="p-4 bg-white rounded-lg border">
                                                ...
                                                {settings.logoPath && (
                                                    <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded text-xs text-green-700">
                                                        ✓ Logo is currently set and will appear on BOT
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                    ...
                    {activeTab === 'body' && (
                        <SectionCard
                            ...
                            description="Choose which columns to display in the BOT products table"
                    ...
                    {activeTab === 'footer' && (
                        <SectionCard
                            ...
                            description="Configure footer information for BOT"
...
const ToggleSwitch = ({ enabled, onChange, disabled = false }) => (
    <button
        ...
        disabled={disabled}
        className={`relative inline-flex h-5 w-10 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-submit focus:ring-offset-1 ${disabled ? 'bg-gray-200 cursor-not-allowed' : enabled ? 'submit' : 'bg-gray-300'
            }`}
    >
        <span
            className={`inline-block h-4 w-4 transform rounded-full shadow transition-transform ${disabled ? 'bg-gray-300' : 'bg-white'
