...
const CombinedSalesChart = () => {
    ...
    }, [ selectedPeriod, chartType, monthlySalesData, weeklySalesData, dailySalesData, monthlyPurchaseData, weeklyPurchaseData, dailyPurchaseData, currency]);
    ...
    const fillMissingWeeklyData= (data) => {
        const labels = formatWeeklyLabels(); // ["Week 35", "Week 36", ...]
        const labelMap = new Map(labels.map((label, idx) => [label.split(' ')[1], idx])); // map week number -> index

        const filledData = Array(labels.length).fill(0).map(() => ({ totalSales: 0, totalPurchases: 0 }));

        data.forEach(item => {
            const idx = labelMap.get(item.week.toString());
            if (idx !== undefined) {
                filledData[idx] = {
                    totalSales: item.totalSales,
                    totalPurchases: item.totalPurchases
