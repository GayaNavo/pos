...
export default function ReportHeader({ userData, sidebarHidden, setSidebarHidden }) {
    ...
    const defaultAvatar = avatar; // Use local avatar image as fallback
                ...
                {({ open }) => (
                    <>
                        <div className="flex h-full items-center justify-between px-2 sm:px-3 lg:px-8">
                            {/* Logo and Sidebar Toggle Button on Left */}
                            <div className="flex items-center space-x-2 sm:space-x-4">
                                ...
                                {/* Sidebar Toggle Button for Tablet View - Moved after logo */}
                                <button
                                    onClick={() => setSidebarHidden && setSidebarHidden(!sidebarHidden)}
                                    className="hidden sm:flex 2xl:hidden p-1 lg:p-2 rounded-lg border border-gray-300 hover:bg-gray-100 transition-colors duration-200"
                            ...
                            <div className="flex items-center space-x-2 sm:space-x-4">
                                {/* Mobile Menu Button */}
                                <Disclosure.Button className="lg:hidden inline-flex items-center justify-center rounded-md p-2 text-black hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-black">
                                ...
                                <div className="hidden sm:flex p-1 sm:p-2 w-[35px] h-[35px] sm:w-[40px] sm:h-[40px] sm:mr-2 lg:mr-5 border button-bg-color button-hover-color rounded-[14px] items-center justify-center">
                                    <button className='' onClick={handleFullScreen}>
                                        <img className="w-[20px] h-[18px] sm:w-[25px] sm:h-[23px]" src={Full} alt="" />
                                    </button>
                                </div>

                                <button
                                    className="submit hidden sm:flex items-center text-center rounded-full px-3 sm:px-4 lg:px-5 py-2 sm:py-3 pt-[7px] h-8 sm:h-10 mr-2 lg:mr-5 text-xs sm:text-sm text-white shadow-lg hover:shadow-xl focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 w-[80px] sm:w-[96px] text-center"
                                    onClick={handlePOSClick}
                                >
                                    <img className="w-[25px] h-[22px] sm:w-[35px] sm:h-[30px] ml-[2px]" src={PosIcon} alt="pos" />
                        ...
                        <Disclosure.Panel className="lg:hidden">
                            <div className="space-y-1 px-2 pt-2 pb-3 bg-white shadow-md">
                                {navigation.map((item) => (
                                    <Disclosure.Button
                                        key={item.name}
                                        as={Link}
                                        to={item.href}
                                        className={`${linkClasses(item.href)} w-full text-left`}
                                    >
                                        {item.name}
                                    </Disclosure.Button>
                                ))}
