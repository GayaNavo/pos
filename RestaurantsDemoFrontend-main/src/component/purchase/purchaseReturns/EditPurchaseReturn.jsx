...
function EditPurchaseReturnBody() {
    ...
    const calculateTotal = () => {
        ...
        const discountAmount = discountType === 'percentage'
            ? (subtotal * ((saleReturnData.discount || 0) / 100))
            : (saleReturnData.discount || 0);

        // Step 3: Shipping
        const shipping = parseFloat(saleReturnData.shipping) || 0;

        // Step 4: Tax (applied after discount)
        const taxableAmount = subtotal - discountAmount;
        const overallTaxRate = saleReturnData.tax ? parseFloat(saleReturnData.tax) / 100 : 0;
        const taxAmount = taxableAmount * overallTaxRate;
                        ...
                        <div className="flex w-full space-x-5"> {/* Add space between inputs if needed */}
                            ...
                            <div className="flex-1 ">
                                ...
                                <input
                                    ...
                                    required
                                    disabled
                                    value={saleReturnPayingData.date?.dateOnly || ""}
