...
export const getTaxHandler = (product) => {
    if (product.warehouse && typeof product.warehouse === 'object') {
        const taxTypes = Object.values(product.warehouse)
            .flatMap((warehouse) =>
                warehouse.variationValues
                    ? Object.values(warehouse.variationValues).map(v => v.taxType)
                    : [warehouse.taxType]
            )
            .filter(taxType => typeof taxType === 'string' && taxType.trim() !== '');

        return taxTypes.length > 0 ? taxTypes[0] : "none";
    }

    const taxType = product.taxType;
    return (typeof taxType === 'string' && taxType.trim() !== '') ? taxType : "none";
};
