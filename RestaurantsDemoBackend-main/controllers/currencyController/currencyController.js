...
const updateCurrency = async (req, res) => {
    const { id } = req.params;
    const { editcurrencyName, editcurrencyCode, editcurrencySymbole } = req.body;

    const currencyName = editcurrencyName?.trim();
    const currencyCode = editcurrencyCode?.trim();
    const currencySymbole = editcurrencySymbole?.trim();

    if (!id) {
        return res.status(400).json({ message: "Currency ID not found in request.", status: "unsuccess" });
    }
    if (!currencyName || !currencyCode || !currencySymbole) {
        return res.status(400).json({ message: "All fields are required.", status: "unsuccess" });
    }

    try {
        const objectId = new mongoose.Types.ObjectId(id);
        
        // Check if any other currency (excluding the current one) has duplicate values
        const existingCurrency = await Currency.findOne({
            $and: [
                ...
                { _id: { $ne: objectId } } // Exclude the current document being updated
            ]
        });

        if (existingCurrency) {
            let duplicateField = '';
            if (existingCurrency.currencyName === currencyName) duplicateField = 'Currency name';
            else if (existingCurrency.currencyCode === currencyCode) duplicateField = 'Currency code';
            else if (existingCurrency.currencySymbole === currencySymbole) duplicateField = 'Currency symbol';
        ...
        const updatedCurrency = await Currency.findByIdAndUpdate(
            id,
            { currencyName, currencyCode, currencySymbole },
            { new: true, runValidators: true }
        );

        if (!updatedCurrency) {
            return res.status(404).json({ message: 'Currency not found.', status: 'unsuccess' });
        }
