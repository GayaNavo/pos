...
const createQuatation = async (req, res) => {
    try {
        ...
        const productsData = qatationData.productsData;
...
const DeleteQuatation = async (req, res) => {
    ...
    try {
        const deletedQuatation = await Quatation.findByIdAndDelete(id);
        if (!deletedQuatation) {
            return res.status(404).json({ message: 'Quatation not found' });
        }
        res.status(200).json({ message: 'Quatation deleted successfully!', quatation: deletedQuatation });
    } catch (error) {
...
const updateQuatation = async (req, res) => {
    try {
        ...
        const updatedFields = {
            ...existingQuatation.toObject(),
            ...updateData,
            warehouse: existingQuatation.warehouse,
            customer: existingQuatation.customer,
        };
...
const deleteProductFromQuatation = async (req, res) => {
    const { quatationID, productID, total } = req.query;
