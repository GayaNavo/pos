...
const generateBOTTemplate = (saleData, botSettings, settings, req) => {
  const { header, body, footer, general } = botSettings;
  const fontSize = general?.fontSize || '13px';
  const fontFamily = general?.fontFamily || 'Arial, sans-serif';
  ...
  if (header && header.enabled) {
      ...
      if (logoField && botSettings.logoPath) {
        const baseUrl = `${req.protocol}://${req.get('host')}`;
        const logoUrl = `${baseUrl}/api/bot-logo/${botSettings.logoPath.split('/').pop()}`;
        template += `
...
module.exports = { generateBOTTemplate };
